...
<script>
      $(document).ready(function() {
        ...
        // EB Death ESSENTIAL (KAJAL)
        /* ------------------------ NOTE: EB DEATH CAN COME IN INVESTIGATION, PROPERTY ALSO------------------ */

        $("#eb_deathcase_investigation_submit").click(function() {
            var editButtonText = $(this).val();
            if (editButtonText === "Next") {
                $("#generate_ila").css('display', 'none');
                $('#eb_deathcase_investigation').trigger('submit');
        ...
        if ($("#eb_deathcase_investigation_submit").val() === "Edit") {
            console.log($("#eb_deathcase_investigation_submit").val())
            $(".disabledbtn").addClass('disabled-btn');
            $(".editable-field").prop("disabled", true); // updated selector
            $("#toggleManualEntry").prop("disabled", true);
        }
        $("#eb_deathcase_investigation").validate({
            errorClass: 'error',
            errorElement: 'div',
            ...
            submitHandler: function(form) {
                ...
                $('.form-group', '#invoiceContainer').each(function(index, element) {
                    ...
                    // Get type from hidden field or JS var
                    var assignmentType = $(".assignmentType").val(); // or use the JS variable directly

                    // Decide the URL
                    var ajaxUrl = '';
                    if (assignmentType === 'outgoing') {
                        ajaxUrl = '<?php echo base_url("ebdeathessential_outgoing"); ?>';
                    } else {
                        ajaxUrl = '<?php echo base_url("ebdeathessential"); ?>';
                    }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status == 200) {
                            ...
                            $('#eb_deathcase_investigation').val("Edit");
