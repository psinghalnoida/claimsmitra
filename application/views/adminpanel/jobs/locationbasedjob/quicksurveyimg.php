                            ...
                            <div class="panel">
                                <div class="panel-content ml-2 mr-2">
                                    ...
                                    <div class="row gallery">
                                        <?php if (is_array($quicksurveyimages) && !empty($quicksurveyimages)): ?>
                                         <?php $index = 0; // Initialize index variable ?>
                                            <?php foreach ($quicksurveyimages as $image): ?>
                                                <div class="col-6 col-sm-4 col-md-3 col-lg-2 mb-3 mt-2">
                                                    <a href="<?php echo base_url('quicksurvey/' . $directoryname . '/images/' . $image); ?>" data-fancybox="images">
                                                        <div class="card-img-container">
                                                            <img src="<?php echo base_url('quicksurvey/' . $directoryname . '/images/' . $image); ?>" class="card-img-top" alt="Image <?php echo $index; ?>" style="height:100%;">
                                                        </div>
                                                    </a>
...
<script>
    $(document).ready(function() {
       ...
       $('#images').on('change', function() {
            ...
            var directoryname = "<?php echo $directoryname; ?>"; // PHP variable passed to JS

            $.each(files, function(i, file) {
                formData.append('images[]', file); // Append each file to FormData
            });

            formData.append('directoryname', directoryname);  // Add directory name to FormData
            formData.append('filetype', "images");  // Add filetype to FormData

            $('#progressModal').modal('show');  // Show progress modal

            $.ajax({
                url: '<?php echo base_url('cases/uploadquicksurveyimages'); ?>',
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
