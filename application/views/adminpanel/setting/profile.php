                            ...
                            <div class="tab-pane fade <?= ($active_tab == 'bank_information') ? 'show active' : '' ?>" id="bank_information" >
                                <div class="nav-item dropdown nav--user online" style="text-align:right">
                                    ...
                                    <div class="dropdown-menu" id="bank" style="width:auto;">
                                        <a href="javascript:void(0);" class="dropdown-item" id="openBankModal" >
                                            <i class="fas fa-list"></i> Add bank
                                        </a>
                                    ...
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content ">
                                            <div class="modal-body text-center">
                                                <div id="drop-area" style="border: 2px dashed #aaa; padding: 60px; border-radius: 5px;">
                                                    <label for="fileInput" class="btn btn-light" style="border:none;">
                                                        <img src="<?= base_url('assets/cloud-computing.png') ?>" alt="" width="10%"></br>
                                                        <input type="file" id="fileInput" multiple accept=".jpg, .jpeg, .png" style="display: none;">
                                                        <p id="fileName" class="mb-0"></p>
                                                    </label>
                                                    <p style="color:black;">Browse cancel cheque to update</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
$(document).ready(function() {
 ...
 document.getElementById('openBankModal').addEventListener('click', function () {
    // Remove existing modal (if any) to avoid duplicates
    const existingModal = document.getElementById('vCenteredModal');
    if (existingModal) {
    ...
    // Append fresh modal
    const modalHtml = `
        <div id="vCenteredModal" class="modal fade" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-body text-center">
                        <div id="drop-area" style="border: 2px dashed #aaa; padding: 60px; border-radius: 5px;">
    ...
    const modalInstance = new bootstrap.Modal(document.getElementById('vCenteredModal'));
    modalInstance.show();

    // Drag-and-drop + file input setup (after modal added)
        ...
        canvas.toBlob(function(blob) {
            ...
            $.ajax({
                ...
                if (response.status === "success") {
                    ...
                    $('#profile_image_preview').attr('src', response.image_url + '?t=' + new Date().getTime());
                } else {
